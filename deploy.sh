...
PROJECT_NAME="immigration-chatbot-$(date +%Y%m%d%H%M%S)"
...
# --------------------------------------------------
# 3. Register GitHub credentials with CodeBuild
# --------------------------------------------------
...
if [ $? -eq 0 ]; then
  echo "✓ GitHub credentials imported."
else
  echo "✗ Failed to import GitHub credentials."
  exit 1
fi
